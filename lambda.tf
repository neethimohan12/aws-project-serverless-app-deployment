...
resource "aws_lambda_function" "invoke_dynamodb" {
  ...
  function_name    = "${var.lambda_function_name_prefix}_dynamodb"
  role             = aws_iam_role.lambda_role_dynamodb.arn
  handler          = "app.lambda_handler" //app is the file name app.py
...
resource "aws_lambda_function" "invoke_s3" {
  ...
  function_name    = "${var.lambda_function_name_prefix}_s3"
...
resource "aws_lambda_function" "invoke_glue" {
  ...
  function_name    = "${var.lambda_function_name_prefix}_glue"
